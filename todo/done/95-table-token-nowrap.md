...
- nowrap 지정 추가: settings policyId("POL-/0001" 실측)·activeFrom/To·수정일시×2 ·
  recent-ops occurredAt(3줄 실측)·entityId · book-detail acquiredAt·운영기록 occurredAt.
- 요약·설정값·회원유형 라벨 등 자연 개행이 맞는 열은 그대로(공백 개행만 발생).
- 좁은 창 재캡처: 처리 시각·정책 ID 한 줄 유지(/tmp/vis/95-crop*.png). 전 게이트 · 12 e2e.
