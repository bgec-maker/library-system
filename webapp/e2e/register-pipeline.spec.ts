...
import { MOCK_API_URL, REGISTER_ISBN, REGISTER_BARCODE, installApiMock } from './mockApi';
...
// todo/118 — 필수 미충족 시 "배너만"이 아니라 해당 입력을 직접 가리킨다(포커스+aria-invalid,
// 입력 시 해제). 무ISBN 폼의 취소 라벨은 「취소」(스캔 경로 문구 공유 오류 해소)인 것도 함께.
test('무ISBN 폼 — 빈 저장 시 서명 포커스·aria-invalid, 입력하면 해제, 취소 라벨', async ({ page }) => {
  ...
  await page.locator('.dock-icon[title="도서 등록"]').click();
  await page.getByRole('button', { name: 'ISBN 없이 수동 등록' }).click();
  await expect(page.locator('#regMTitle')).toBeVisible();
  await expect(page.getByRole('button', { name: '취소', exact: true })).toBeVisible();

  await page.getByRole('button', { name: '저장', exact: true }).click();
  await expect(page.locator('#regMTitle')).toBeFocused();
  await expect(page.locator('#regMTitle')).toHaveAttribute('aria-invalid', 'true');

  await page.locator('#regMTitle').fill('테스트 서명');
  await expect(page.locator('#regMTitle')).not.toHaveAttribute('aria-invalid', 'true');

  // 서명만 채우고 저장 → 이번엔 저자 필드가 지목된다
  await page.getByRole('button', { name: '저장', exact: true }).click();
  await expect(page.locator('#regMAuthors')).toBeFocused();
  await expect(page.locator('#regMAuthors')).toHaveAttribute('aria-invalid', 'true');
});
