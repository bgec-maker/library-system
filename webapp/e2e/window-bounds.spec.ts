...
test('창 경계 — 드래그 탈출 불가·오염 복원 교정·서 리사이즈 앵커·뷰포트 축소', async ({ page }) => {
  ...
  await page.locator('.dock-icon[title="대출·반납"]').click();
  await page.waitForTimeout(250); // 등장 애니메이션(120ms) 정착 후 측정 — 중간 프레임은 scale 0.98
  ...
  await page.waitForTimeout(250); // 등장 정착
...
// todo/131 「창 상호작용 계약」 — ① 도크 재클릭은 복제가 아니라 포커스(비단일 뷰 포함)
// ② 타이틀바 더블클릭 = 최대화 토글(prevRect 왕복) ③ 스냅은 자유 배치 저장을 덮지 않는다.
test('창 상호작용 — 도크 재클릭 포커스·더블클릭 최대화 왕복·스냅 비저장', async ({ page }) => {
  await installApiMock(page);
  await page.goto('./');
  await page.fill('#sg-url', MOCK_API_URL);
  await page.fill('#sg-token', 'e2e-token');
  ...
  // ① 비단일 뷰(학생 관리) 도크 2회 클릭 — 창은 1개(종전엔 클릭마다 복제)
  ...
  // 자유 배치 확보 — e 핸들로 +60px 리사이즈(이 rect가 localStorage에 저장된다)
  const eHandle = win.locator('.window-resize--e');
  let hb = (await eHandle.boundingBox())!;
  await page.mouse.move(hb.x + 2, hb.y + hb.height / 2);
  await page.mouse.down();
  await page.mouse.move(hb.x + 62, hb.y + hb.height / 2, { steps: 4 });
  await page.mouse.up();
  const freeRect = (await win.boundingBox())!;

  // ② 더블클릭 최대화 → 워크스페이스 전체, 재더블클릭 → 자유 배치 복원(±2px)
  const bar = win.locator('.window-titlebar');
  await bar.dblclick();
  let bb = (await win.boundingBox())!;
  expect(Math.abs(bb.x - DOCK_W)).toBeLessThanOrEqual(2);
  expect(Math.abs(bb.y - 0)).toBeLessThanOrEqual(2);
  expect(Math.abs(bb.width - (viewport.width - DOCK_W))).toBeLessThanOrEqual(2);
  expect(Math.abs(bb.height - viewport.height)).toBeLessThanOrEqual(2);
  await bar.dblclick();
  bb = (await win.boundingBox())!;
  expect(Math.abs(bb.width - freeRect.width)).toBeLessThanOrEqual(2);
  expect(Math.abs(bb.x - freeRect.x)).toBeLessThanOrEqual(2);

  // ③ 스냅(왼쪽 절반) 후 닫고 재열기 — 자유 배치로 열린다(스냅 rect가 저장을 덮지 않음)
  await win.getByRole('button', { name: '왼쪽 절반' }).dispatchEvent('click');
  // 절반(602)이 학생 관리 최소 폭(registry min 640)보다 작으면 최소 폭이 이긴다 — 스냅은
  // resizeWindow를 경유하므로 meta.min 계약이 그대로 적용된다(과축소 금지가 스냅보다 우선).
  const half = Math.max(Math.floor((viewport.width - DOCK_W) / 2), 640);
  bb = (await win.boundingBox())!;
  expect(Math.abs(bb.width - half)).toBeLessThanOrEqual(2);
  await win.getByRole('button', { name: '닫기' }).click();
  await page.locator('.dock-icon[title="학생 관리"]').click();
  await page.waitForTimeout(250); // 등장 정착
  bb = (await page.locator('.window').first().boundingBox())!;
  expect(Math.abs(bb.width - freeRect.width)).toBeLessThanOrEqual(2);
});
