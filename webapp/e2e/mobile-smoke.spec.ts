...
test('모바일 셸 — 탭바 4칸 · 대출 흐름 · 등록 파이프라인 · 더보기→리포트 push', async ({ page }) => {
  ...
  await test.step('카메라 무대 — body 포털 + 탭바 위 전면(todo/46 회귀) + 미인식 힌트(todo/68)', async () => {
    ...
    // todo/81 — 유효 대상 주입 시 스크린리더 낭독문이 사람 말로 채워진다(미인식은 침묵 — 위
    // '???' 주입 후에도 이 영역은 비어 있었어야 한다는 것까지 함께 보증).
    await page.evaluate(() => {
      (window as unknown as { __e2eScan: (raw: string) => void }).__e2eScan('0001230');
    });
    await expect(stage.locator('.sr-only[role="status"]')).toHaveText('등록번호 0001230 인식');
  ...
  await test.step('todo/97 — 로케일 전환 시 모바일 헤더 제목 생동성(탭·스택)', async () => {
    await page.goBack();
    await expect(page.locator('.m-stack-overlay')).toHaveCount(0);

    await page.locator('.m-more-locale-btn', { hasText: 'EN' }).click();
    // 탭 헤더(더보기 루트) — 동결됐던 결함 지점: 라벨·목록은 영어인데 헤더만 「더보기」로 남았었다.
    await expect(page.locator('.m-shell-title')).toHaveText('More');
    // 스택 push 제목도 새 언어로.
    await page.locator('.m-more-item', { hasText: 'Reservations' }).click();
    await expect(page.locator('.m-stack-title')).toHaveText('Reservations');
    // 스택이 열린 채 KO 복귀 — 열려 있는 엔트리의 제자리 재매핑(비커스텀) 검증.
    await page.goBack();
    await expect(page.locator('.m-stack-overlay')).toHaveCount(0);
    await page.locator('.m-more-locale-btn', { hasText: 'KO' }).click();
    await expect(page.locator('.m-shell-title')).toHaveText('더보기');
  });
});
