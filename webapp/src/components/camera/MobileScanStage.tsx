...
const TICK_MS = 250;
// todo/68(M2 바코드 패턴 still-searching) — 이 시간 동안 디코드가 하나도 없으면 조작 힌트를
// 보여준다. 초보 사서의 흔한 원인 두 가지(너무 멀다·너무 어둡다)만 짧게.
const MISS_HINT_MS = 6000;
...
export function MobileScanStage({ viewId }: MobileScanStageProps) {
  ...
  // setTimeout 재무장(성능 예산 규칙과도 정합).
  const armMissTimer = useCallback(() => {
    if (missTimerRef.current) clearTimeout(missTimerRef.current);
    setMissHint(false);
    missTimerRef.current = setTimeout(() => setMissHint(true), MISS_HINT_MS);
  }, []);
  ...
  // todo/68 — 카메라가 실제로 도는 동안만 힌트 타이머를 돌린다(준비 중·오류엔 무의미).
  useEffect(() => {
    if (status.state === 'active') armMissTimer();
    return () => {
      if (missTimerRef.current) clearTimeout(missTimerRef.current);
    };
  }, [status.state, armMissTimer]);
  ...
  return createPortal(
    <div className="scan-stage" ref={containerRef}>
      ...
      {missHint && !recognizedText && (
        <div className="scan-stage__missHint" role="status">
