...
## todo/133~135 · 박스 이탈 점검 (2026-07-19)

- **todo/93·95의 nowrap 계약 재정의** — 그때 "1fr 트랙의 자동 최소값이 nowrap 값의 폭으로
  올라가 그리드가 스스로 넓어진다"를 해법으로 썼는데, 그 같은 성질이 20자 토큰 두 쌍에선
  뷰포트 관통이 됐다(실기기 제보). 이번 재정의: 트랙 바닥 제거(minmax(0,1fr))가 기본,
  nowrap엔 ellipsis 안전망, 불투명 ID는 nowrap 대신 anywhere 접힘(무손실). 짧은 정상값의
  렌더는 종전과 동일해 93·95의 실측 계약은 그대로 성립한다.
- **entityId만 nowrap을 뗀 이유** — 날짜(occurredAt)는 중간 꺾임이 의미를 해치는 짧은 값
  (nowrap 유지·안전망 추가), ID는 접혀도 대조에 지장이 없고 말줄임은 뒷자리를 지운다.
- **검출기 범위** — 문서·셸 스크롤러 scrollWidth 초과만 상주 단정. 요소 rect 스캔은 스윕
  (134)용 — fixed·의도적 오프캔버스 요소 필터가 화면마다 달라 상주 회귀엔 과하다.

---

## todo/136~138 · 도움말·공지 탭 (2026-07-19)

- **공지 저장소 = 시트, 표시 = 웹앱** — 사용자의 "시트에 남기는 게 아니지?"는 읽는 표면에
  대한 요구로 해석했다(사서는 시트를 안 본다). 저장까지 레포로 가져오면 공지 한 줄마다
  빌드·배포가 필요해 공지의 존재 이유(즉시 게시)가 죽는다 — 시트 행 추가는 관리자 폰에서
  10초고, 이 시스템의 데이터는 원래 전부 시트다.
- **가이드는 반대로 번들 내장** — 사용법은 기능과 같은 순간에 바뀌어야 한다(기능 커밋에
  가이드 갱신 동봉 규약). i18n JSON이 아니라 콘텐츠 모듈(ko/en)인 이유·리터럴 게이트와의
  관계는 src/content/helpTypes.ts 머리 주석이 정본.
- **부팅 토스트와 lastSeen의 분리** — 토스트를 본 것과 공지를 읽은 것은 다르다: lastSeen은
  도움말 열람 시에만 갱신 → 안 읽으면 다음 부팅에 또 안내(귀찮음이 아니라 전달 보장).
  배지 인프라 신설은 보류(최소 표면 — 필요 제보 시 후속).
- **e2e 고정 대기 30초 함정** — reload 후 "게이트가 있을 수도"라며 fill().catch()를 쓰면
  없는 셀렉터 대기 30초가 테스트 예산을 태워 진짜 단정이 시간 부족으로 죽는다(재현·교정).
  조건부 UI는 존재 확인 후 행동으로.
