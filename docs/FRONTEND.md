...
- **스캐너는 창이 아니다**: 우하단 고정 도크 위젯. 닫기 불가, 접기만
- **카메라는 온디맨드 (ADR-020 — '상시 구동' 반전)**: 기본 꺼짐. 시작 = 위젯 클릭·스캔 뷰 시작 버튼·단축키 S / **유휴 3분 자동 종료**(10초 전 토스트 예고) / **연속 모드 핀** = 러시아워용 유휴 타이머 해제(위젯에 표시) / 모바일 = 스캔 탭 진입 시 시작 버튼, 탭 이탈 시 즉시 종료. `getUserMedia` 단일 호출점·ref-count는 그대로
