...
| 3 | todo/125 회원 웹 API | doPost 4액션: memberList/memberRegister/memberUpdate/classCodes (도메인 함수 무수정 재사용) | 학생 관리 뷰가 「서버 업데이트 필요」 안내를 표시(UNKNOWN_ACTION 폴백) — 무해 |
| 4 | todo/136 공지 저장소 | 23_NOTICES 시트(업그레이드 메뉴가 생성) + notices 읽기 액션 — 도움말 탭의 공지 원천 | 도움말 탭에서 공지 섹션만 「서버 업데이트 후 표시」 한 줄 — 사용법 가이드는 정상 |

**배포 후 조치(1회)**: 스프레드시트 새로고침 → 📚 도서관 관리 → 관리 → **「스키마 업그레이드」 실행** (birth_year 열 추가 + CLASS 코드군 시드 LOVE/HOPE/FAITH + 검증 리스트
...
## todo/125 · 회원 웹 API (2026-07-19) — 재배포 필요 🅿 (대장 #3)

**추가분(기존 함수 무수정)**: doPost 디스패치 4줄 + `apiWebMemberList_`/`apiWebMemberRegister_`/
`apiWebMemberUpdate_`/`apiWebClassCodes_`.

- memberList: 검색(이름 normalizeText_ 부분일치·회원번호 cleanCode_ 부분일치)·반·상태 필터,
  반 sort_order→이름 정렬, 대출중 건수 동봉, birthYearReady(마이그레이션 여부 신호), 상한 500.
- 쓰기 2종은 사이드바와 같은 executeWrite_ 멱등 래핑(requestId). ensureOperatorNote_는 의도적
  미사용 — 회원 note는 원장 데이터라 반 이동마다 작업자명이 쌓이면 오염(감사는 15_AUDIT_LOG).
- PII 최소화: 목록 응답에 전화·이메일·학번 미포함.

## todo/136 · 공지 저장소 (2026-07-19) — 재배포 필요 🅿 (대장 #4)

**추가분**: HEADERS 23_NOTICES + 업그레이드 메뉴(라벨을 「스키마 업그레이드」로 일반화 —
반·생년+공지 시트 누적 수행)에 시트 생성 통합 + `apiWebNotices_`(읽기 전용) + doPost 1줄.

**공지 올리는 법(관리자)**: 스프레드시트 23_NOTICES 탭에 행 추가 — **title·body만 채우면
즉시·상시·INFO 공지**. 선택: level=WARN(주황 강조), pinned=TRUE(상단 고정), starts_at/
ends_at(게시 기간), status_code를 ACTIVE 외 값으로 바꾸면 숨김(초안·내림). 저장하면 사서
웹앱 도움말 탭에 바로 뜬다(배포 불요) — 새 공지는 사서 앱 부팅 시 토스트로 1회 안내된다.
